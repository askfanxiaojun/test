...
<html lang="zh-cn">
<head>
	<title><?php echo ($iteminfo["name"]); ?> - BUPT众筹</title>
		...
		<div class="x12 margin-big-top">
				...
				<div class="tab">
					...
					<div class="x12 tab-body bg-white padding-big">
						...
						<div class="tab-panel active" id="tab-a">
							...
							<div class="x12">
								...
								<div class="x12 margin-bottom">
									...
									<?php if(!$_SESSION['user']['uin']): ?><span class="x12 text-center"><span class="padding-right text-large icon-exclamation text-red"></span>您没有权限查看此信息! 请<a class="text-red" href="<?php echo U('User/Login/index',array('bakurl'=>base64_encode('/index.php?s=/Item/info/id/3.html')));?>">登录</a></span>
										<?php else: ?>
							...
							<div class="x12">
								...
								<?php if(is_array($video)): $i = 0; $__LIST__ = $video;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 margin-bottom">
										<h3 class="padding"><?php echo ($bupt["pname"]); ?></h3>
										<embed src="<?php echo ($bupt["url"]); ?>" quality="high" width="730" height="400" align="middle" allowScriptAccess="always" allowFullScreen="true" mode="transparent" type="application/x-shockwave-flash"></embed>
						...
						<div class="tab-panel" id="tab-b">
							...
							<?php if(is_array($invest)): $i = 0; $__LIST__ = $invest;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 text-center padding border-bottom border-red-light">
									<span class="x4"><?php echo ($bupt["user_name"]); ?></span>
									<span class="x4">￥<?php echo ($bupt["money"]); ?></span>
									<span class="x4"><?php echo (date("Y-m-d",$bupt["create_time"])); ?></span>
								</div><?php endforeach; endif; else: echo "" ;endif; ?>
						</div>
						<!-- 项目动态 -->
								...
								<div class="timeline-date">
									<ul>
										...
										<?php if(!$log): ?><dl class="right ">
											...
											<?php if(is_array($log)): $i = 0; $__LIST__ = $log;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><li>
													<h3><?php echo (date("m-d",$bupt["time"])); ?><span><?php echo (date("Y",$bupt["time"])); ?></span></h3>
													<dl class="right ">
														<span><?php echo ($bupt["content"]); ?></span>
													</dl>
						...
						<div class="tab-panel" id="tab-d">
							...
							<?php if(is_array($questionsList)): $i = 0; $__LIST__ = $questionsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 margit-top border-bottom border-back padding">
									<div class="x2">
										<img src="<?php echo ($bupt["header"]); ?>" alt="<?php echo ($bupt["u_name"]); ?>" width="88" height="88" class="radius-circle">
									</div>
									<div class="x10 height">
										<span class="x12 text"><strong><?php echo ($bupt["u_name"]); ?>：</strong></span>
										<span class="x12 text-small padding-left"><?php echo ($bupt["content"]); ?></span>
									</div>
								</div><?php endforeach; endif; else: echo "" ;endif; ?>
						</div>
					</div>
			...
			<div class="x4 padding-left ">
				...
				<div class="x12 bg-white padding-big margin-big-top ">
					...
					<?php if(is_array($leaduser)): $i = 0; $__LIST__ = $leaduser;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 margin-big-top">
							<span class="x4 padding-big"><img src="<?php echo ($bupt["header"]); ?>" alt="alt" width="80" height="80"class="radius-circle"></span>
							<span class="x8 padding-big">
								<h1><?php echo ($bupt["user_name"]); ?></h1>
								<span class="x12 margin-top text-small text-gray height">
									<span class="x12">认投金额：<?php echo ($bupt["countmoney"]); ?></span>
									<span class="x12">认投时间：<?php echo (date("Y-m-d",$bupt["time"])); ?></span>
								</span>
							</span>
						</div><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				...
				<div class="x12 bg-white margin-big-top padding-big">
					<h1>Q&A</h1>
					<?php $info = D("Home/News")->loadList("a.status =1 AND a.cid In(5)","10","a.time desc");foreach ($info as $i=>$bupt):$bupt["url"]=U("news/info",array("id"=>$bupt["id"]));?><a href="<?php echo ($bupt["url"]); ?>" class="x12 margin-big-top text-red">Q：<?php echo ($bupt["title"]); ?></a><?php endforeach ?>
				</div>
			</div>
		</div>
	</div>
...
function collect(itemid){
	$.get("/User/Login/check_login",{bakurl:"<?php echo base64_encode('/index.php?s=/Item/info/id/3.html');?>" }, function(d){
		if (d.status==1) {
			//如果检测有登录状态
			$.post("/index.php?s=/Home/Item/collect", {
				itemid: itemid
				},function(d){
					if (d.status==1) {
...
function interview(itemid){
	$.post("/User/Login/check_login", {
	 	bakurl:"<?php echo base64_encode('/index.php?s=/Item/info/id/3.html');?>"
	},function(d){
		if (d.status==1) {
...
function sub_interview(itemid){
	...
	$.post("/index.php?s=/Home/Item/interview_user", {
	 	content: content.val(),
	 	itemid: itemid
	},function(d){
		if (d.status == 1) {
...
function lead_check(itemid){
	$.get("/index.php?s=/Home/Item/lead_check",{
		itemid:itemid
	}, function(d){
		if (d.status==1) {
			open_lead(itemid)
...
function sub_lead(itemid){
	...
	$.post("/index.php?s=/Home/Item/lead_user", {
...
function sub(a){
	...
	$.post("/index.php?s=/Home/Qa/questions_in", {
	 	title: title.val(),
	 	content: content.val(),
