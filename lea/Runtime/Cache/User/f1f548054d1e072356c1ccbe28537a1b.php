...
<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		<div class="x12 bg-white padding height-big">
			<div class="x7">
				<span class="float-left padding-big-left padding-big-right  border-right"><?php echo ($bupt["title"]); ?></span>
				<span class="float-left padding-big-left padding-big-right text-small border-right">类型：
					<?php if($bupt['type'] == 1): ?>进账<?php else: ?>出账<?php endif; ?></span>
				<span class="float-left padding-big-left padding-big-right text-small border-right"><span class="text-red"><?php if($bupt['type'] == 1): ?>+<?php else: ?>-<?php endif; echo ($bupt["money"]); ?></span></span>
				<div class="x12 text-gray bg-white border-top text-small"><span class="text padding-left">备注：</span><?php echo ($bupt["remark"]); ?></div>
			</div>

			<div class="x5">
				<div class="x8">
					<span class="x12 padding-big-left padding-big-right  text-small border-right">创建时间：<?php echo (date("Y-m-d h:i:s",$bupt["create_time"])); ?></span>
					<span class="x12 padding-big-left padding-big-right  text-small border-right"><?php if($bupt['update_time'] ): ?>更新时间：<?php echo (date("Y-m-d h:i:s",$bupt["update_time"])); endif; ?></span>
				</div>
				<div class="x4 text-center" style="margin-top:16px">
					<?php if($bupt['status'] == 1): ?><span class="text-big text-red">成功</span>
						<?php else: ?>
