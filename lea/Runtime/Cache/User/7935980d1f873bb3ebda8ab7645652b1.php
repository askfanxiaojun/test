...
<head>
	...
	<title>银行卡管理 - 会员中心芝麻乐开源众筹系统</title>
...
function select (){
	...
	<div class="x12 padding">
		<button onClick="add_bank(this)"  class="button bg-red float-right radius-none">新增银行卡</button>
	</div>
	<?php if(is_array($bank)): $i = 0; $__LIST__ = $bank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
			<div class="x12 bg-white padding height-big">
				<span class="float-left padding-big-left padding-big-right text-small border-right">姓名：<?php echo ($bupt["name"]); ?></span>
				<span class="float-left padding-big-left padding-big-right  text-small border-right">卡号：<?php echo ($bupt["card"]); ?></span>
				<span class="float-left padding-big-left padding-big-right  text-small border-right">卡类型：<?php echo ($bupt["type"]); ?></span>
				<span class="float-left padding-big-left padding-big-right  text-small border-right">开户行：<?php echo ($bupt["bank"]); ?></span>
			</div>
