			...
			<div class=" padding border-bottom  border-dashed x12">
				...
				<span class="x10">	
					...
					<div class="x12 padding border zhengjian">
						<?php if(is_array($item["prove"])): $i = 0; $__LIST__ = $item["prove"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="text-center" style="background:url(<?php echo ($bupt["url"]); ?>) center; width:100px; height:90px; margin:3px;float:left;">
								<a href="javascript:void(0)" class="text-center bg-red text-white text-small  x12" style="margin-top:75px;background-color:rgba(0,0,0,0.3);" onclick="delpic(this,'zhengjian')">删除</a>
							 	<input type="hidden" name="prove[]" value='<?php echo ($bupt["url"]); ?>' />
