...
<div class="x12 padding">
  <table class="table table-bordered table-hover">
    <tbody>
      ...
      <?php if(is_array($tags)): $i = 0; $__LIST__ = $tags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($vo["id"]); ?>" class="height">
          <td><?php echo ($vo["id"]); ?></td>
          <td id="trname<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></td>
          <td>
            <button class="updates button button-small bg-sub" id="s0<?php echo ($vo["id"]); ?>" url="<?php echo U('/Admin/Item/tags_edit',array('id'=>$vo['id']));?>" sname="<?php echo ($vo["name"]); ?>">编辑</button>
        ...
        <?php if(is_array($vo["lower"])): $k = 0; $__LIST__ = $vo["lower"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rs): $mod = ($k % 2 );++$k;?><tr id="tr<?php echo ($rs["id"]); ?>" class="height">
            ...
            <td>
              ...
              <button class="updates button button-small bg-sub" id="s0<?php echo ($rs["id"]); ?>" url="<?php echo U('/Admin/Item/tags_edit',array('id'=>$rs['id']));?>" sname="<?php echo ($rs["name"]); ?>">编辑</button>
  ...
  layer.config({
    ...
    $('.adds').click(function(){
      ...
      layer.open({
        ...
        content: '<?php echo U('/Admin/Item/tags_add');?>'
