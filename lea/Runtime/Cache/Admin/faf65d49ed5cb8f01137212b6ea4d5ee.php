...
<script src="/Public/lib/layer/layer.js"></script>
      <div class="x12 padding-large">
		...
		<div class="collapse">
		 <?php if(is_array($rule_list)): $i = 0; $__LIST__ = $rule_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="panel">
			<div class="panel-head bg-white "><span class="float-right"><label><input type="checkbox" name="rules[]" onclick="checkd(this,1)" <?php if(in_array($vo['id'],$group_info['rules'])): ?>checked<?php endif; ?> value="<?php echo ($vo["id"]); ?>" /> 选择</label></span> <?php echo ($vo["title"]); ?> </div>
	...
	<script>
		function rules_save(d){
			...
			$.post("/index.php?s=/Admin/Auth/role_edit/id/4",{title:title,rules:rules.join(',',rules)},function(ret){
				if(ret.status==1){
