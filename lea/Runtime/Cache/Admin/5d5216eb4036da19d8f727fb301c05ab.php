...
<div class="padding  x12 ">
	...
	<div class="bg padding ">支付接口配置</div>
		<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="height-large padding border-bottom x12">
			<span class="text-gray x2 padding height"><img src="<?php echo ($bupt["ico"]); ?>" width="23"> <?php echo ($bupt["title"]); ?></span>
			<span class="x8" >
				<?php if($bupt["status"] == 1): if($bupt["partner"] ): echo ($bupt["partner"]); ?>
						<?php else: ?>
						<?php echo ($bupt["business"]); endif; endif; ?>
				<span class="text-gray padding-left"></span>
			</span>

			<span class="x1" >
				<?php if($bupt["status"] == 1): ?><button class="config button border-mix" title="<?php echo ($bupt["title"]); ?>" type="<?php echo ($bupt["type"]); ?>">配置</button><?php endif; ?>
			</span>
			
			<?php if($bupt["status"] == 1): ?><button class="status button border-dot" title="<?php echo ($bupt["title"]); ?>接口卸载" status="uninstall" type="<?php echo ($bupt["type"]); ?>">卸载</button>
				<?php else: ?>
				<button class="status button border-sub" title="<?php echo ($bupt["title"]); ?>接口安装" status="install" type="<?php echo ($bupt["type"]); ?>">安装</button><?php endif; ?>
