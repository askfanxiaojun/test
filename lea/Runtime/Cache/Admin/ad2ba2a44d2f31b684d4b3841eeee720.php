...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bupt): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		...
		<div class="x9 bg padding bg-white border-left height-big">
			<div><span class="x6 text-big"><a href="<?php echo U('Home/Item/info',array('id'=>$bupt['id']));?>"><?php echo ($bupt["name"]); ?></a></span>
			<span class="x6 text-right">
			<button class="button radius-none bg-red"  onclick="iframe('<?php echo U('Admin/Item/item_editstatus',array('itemid'=>$bupt['id']));?>')" type="button"><?php echo ($bupt["progress_name"]); ?></button>
			<button onClick="iframe('<?php echo U('Admin/Item/item_video',array('itemid'=>$bupt['id']));?>')" class="button radius-none bg-sub margin-left">视频管理</button>
			<button onClick="lead(<?php echo ($bupt["id"]); ?>,this)" class="button radius-none bg-yellow margin-left">设置领投规则</button>
			</div>
			<div class="x12">
				<a href="<?php echo U('Admin/Item/item_add',array('itemid'=>$bupt['id']));?>" class="button button-small bg-sub radius-none">修改</a>
				<a href="javascript:void(0)" onclick="delitem('<?php echo U('Admin/Item/item_del',array('id'=>$bupt['id'],'isdel'=>1));?>')" class="radius-none button button-small bg-sub">删除</a>
				<a href="javascript:void(0)" onclick="iframe('<?php echo U('Admin/Item/item_editstatus',array('itemid'=>$bupt['id']));?>')" class="button button-small radius-none bg-green "> 修改项目状态</a>
				<a href="javascript:void(0)" onclick="iframe('/index.php?s=/Admin/Item/interview/itemid/<?php echo ($bupt["id"]); ?>')" class="button button-small bg-sub radius-none"><?php echo ($bupt["interview"]); ?> 位约谈</a>
				<a href="javascript:void(0)"   onclick="iframe('<?php echo U('Admin/Lead/index',array('itemid'=>$bupt['id']));?>')" class="button button-small bg-sub radius-none"><?php echo ($bupt["countlead"]); ?> 位领投</a>
				<?php if($bupt['status'] == '0'): ?><span class="badge bg-yellow-light">未审核</span><?php endif; ?>
			</div>
			<div class="x12 text-gray">
				<div class="x6">
					<span class="x12">融资总额: <span class="text-red">￥<?php echo ($bupt["raising_money"]); ?></span>
					已筹款：<span class="text-red">￥<?php echo ($bupt["countmoney"]); ?></span>
					</span>
				</div>
				<div class="x6">
					<span class="x6">投资方占比: <span class="text-red"><?php echo ($bupt["investment_rate"]); ?>%</span></span>
					<span class="x6">项目发布者: <span class="text-red"><a href="/index.php?s=/Admin/Item/index/uin/<?php echo ($bupt["uin"]); ?>" class="text-sub"><?php echo ($bupt["user_name"]); ?></a></span></span>
				</div>
			</div>
		</div>
