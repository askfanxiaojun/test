...
Class bupt extends TagLib{
	protected $tags   =  array(
		// 通用循环标签
		'buptlist'    =>  array('attr'=>'name,id,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	  	// 定义非闭合标签
...
Eof;
	...
	public function _newslist($attr,$content){
		...
		$str .= 'foreach ($info as $i=>$bupt):';
		$str .= '$bupt["url"]='.'U("news/info",array("id"=>$bupt["id"]));';
		$str .= '?>';
		$str .= $this->tpl->parse($content);
		$str .='<?php endforeach ?>';
		return $str;
    ...
    public function _navlist($attr,$content){
        ...
        $str .= 'foreach ($info as $i=>$bupt):';
        $str .= '$bupt["url"]='.'U("news/index",array("id"=>$bupt["id"]));';
        $str .= '?>';
        $str .= $this->tpl->parse($content);
        $str .='<?php endforeach ?>';
        return $str;
    ...
    public function _nav($attr,$content){
        ...
        $str .= 'foreach ($info as $i=>$bupt):';
        $str .= '$nav2=D("Admin/Nav")->loadList(array("pid"=>$bupt["id"]));';
        $str .= '?>';
        $str .= $this->tpl->parse($content);
        $str .='<?php endforeach ?>';
        return $str;
    ...
    public function _itemlist($attr,$content){
        ...
        $str .= 'foreach ($itemList as $i=>$bupt):';
        $str .= '$bupt["url"]=U("Item/info",array("id"=>$bupt["id"]));';
        $str .= '$bupt["last_time"]=gettime($bupt["end_time"]-time());';
        $str .= '$bupt["success_rate"]=D("User/ItemWithOrder")->successRate($bupt["id"]);';
        $str .= '$bupt["count_money"]=D("User/ItemWithOrder")->countMoney($bupt["id"]);';
        $str .= '?>';
        $str .= $this->tpl->parse($content);
        $str .='<?php endforeach ?>';
        return $str;
	...
	/**
	 * 通用循环标签
     * buptlist 标签解析 循环输出数据集
     */
    public function _buptlist($tag,$content) {
        $name  =    $tag['name'];
        $id    =    $tag['id'];
